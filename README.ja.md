...
PostgreSQLベースのシンプルかつ高速な脆弱性管理APIです。**OSV**・**NIST NVD**・**CISA KEV**・**EPSS**・**ベンダーセキュリティアドバイザリ**などのデータを収集し、正規化されたマスターテーブルを通じて効率的な検索を提供します。

## 特徴

- **マルチソース**: OSV (Open Source Vulnerabilities)・NIST NVD (CVE)・ベンダーアドバイザリ（Fortinet / Palo Alto Networks / Cisco PSIRT 等）に対応
...
- `ecosystem` (必須): エコシステムまたはベンダー (例: `npm`, `PyPI`, `fortinet`)
...
1. **バージョンの整数化**: `1.2.3` → `1002003`（PostgreSQL BigInt）
...
- [x] RESTful API実装
- [x] Prisma ERD生成
- [x] エコシステム全体のバルクダウンロード機能
- [x] バッチ検索API (複数パッケージの一括検索)
- [x] バージョン正規化失敗時のフォールバック検索
- [x] NIST NVD対応（全件ミラー・差分更新・CPEマッピング）
- [x] マルチソース対応 DB スキーマ (OSV/NVD 分離)
- [x] CPE 検索 API (`/search/cpe`)
- [x] Vulnerability マスターテーブル（ソース間重複排除・advisoryId 対応）
- [x] CISA KEV 対応（悪用実績フラグ）
- [x] EPSS 対応（悪用予測スコア）
- [x] CPE ポイントバージョンの正確なバージョン範囲変換
- [x] ベンダーアドバイザリ共通基盤（AdvisoryFetcher インターフェース）
- [x] Fortinet PSIRT 対応（CSAF 2.0 / RSS、バージョンブランチ別管理）
- [x] Palo Alto Networks PSIRT 対応（CSAF 2.0 / RSS、vers:generic/ 形式）
- [x] Cisco PSIRT 対応（openVuln API / OAuth 2.0、CSAF 2.0）
- [x] NVD date range の 120 日制限を自動チャンク分割で解消
- [x] OSV upstream フィールドからの CVE 抽出・NVD マスター統合
- [x] OSV ranges イベントペアリング修正（偽陽性バグ解消）
- [x] ディストリビューション OSV の versions exact match 実装（`introduced: "0"` 偽陽性を根本解消）
- [ ] 定期的なデータ更新のスケジューラ (node-cron)
- [ ] 追加ベンダー対応（F5 BIG-IP、Microsoft MSRC 等）
- [ ] CVE.org 対応
- [ ] GraphQL API対応
- [ ] テストスイートの追加
...
- [ ] CI/CDパイプライン
- [ ] Prometheusメトリクス出力
- [ ] パッケージ名の名寄せ（NVD `xz` vs OSV `xz-utils` 等、ソース間の表記ゆれ統合）
...
**現在の動作**:
- **ecosystem 未指定**: `Ubuntu:*` / `Debian:*` 等のディストリビューション固有エコシステムを検索対象から除外
- **ecosystem 明示指定**（例: `ecosystem=Ubuntu:20.04:LTS`）: dpkg 形式のパッケージバージョン（例: `5.2.4-1ubuntu1`）による `affectedVersions` exact match を使用。upstream バージョン（例: `5.1.1`）は Debian 形式と構造的に一致しないため偽陽性が発生しない

**エコシステム別の検索ロジック**:

| ecosystem の種類 | 検索方式 |
|---|---|
| `Ubuntu:*` / `Debian:*` / `Alpine:*` / `AlmaLinux:*` / `Rocky:*` | `affectedVersions` exact match（dpkg/apk 形式バージョン） |
...
バージョン文字列は `major × 1000000 + minor × 1000 + patch` の整数に変換されます。

| 項目 | 動作 | 影響 |
|---|---|---|
| プレリリース版 (`1.0.0-beta.1`) | 正式版より小さい値として扱う (`1.0.0` - 1) | 軽微な誤差の可能性 |
| ビルドメタデータ (`1.0.0+build.123`) | 除去して無視 | 影響なし |
| 4桁以上のバージョン (`1.2.3.4`) | 4桁目以降を切り捨て (`1.2.3` として扱う) | 同一 major.minor.patch の4桁違いは区別不能 |
| minor/patch が 1000 以上 (`1.1000.0`) | 桁あふれが発生 | 誤った範囲判定の可能性 |
| 各コンポーネントが 999999 超 | 正規化失敗 (null) | フォールバック検索に移行 |
