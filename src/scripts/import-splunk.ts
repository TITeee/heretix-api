...
async function main() {
  console.log('Fetching Splunk security advisories...');

  const job = await prisma.collectionJob.create({
    data: { source: 'advisory-splunk', status: 'running', startedAt: new Date() },
  });

  try {
    const result = await runAdvisoryFetcher(new SplunkFetcher());
