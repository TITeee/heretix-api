...
| Oracle Linux | 99,234 | 6,889,902 | see note | 100.00% | see note |
...
**A note on Oracle Linux's Precision column**: this sweep's TP/FN are stable and reproducible across repeated runs, but FP is not — Oracle's combined `com.oracle.elsa-all.xml.bz2` feed was observed to differ in content between two fetches made minutes apart despite reporting identical byte length, i.e. it's revised in near-real-time far more often than RHEL's per-major feeds are. This is the same "OVAL feeds get revised after the fact" skew documented under [Known limitations](#known-limitations) below, just far more visible here because Oracle Linux ships many single-CVE advisories that each cover dozens of sibling subpackages at once (LibreOffice's `autocorr-<lang>` family alone is ~40 packages) — one in-flight revision multiplies into thousands of individual boundary-point mismatches instead of a handful. Measured Precision has ranged from 86.88% to 87.01% across repeated same-session re-runs (including with sweep concurrency lowered from 20 to 5, which produced byte-for-byte identical results and ruled out request-level contention as the cause); spot-checking the top offending CVEs directly against the live feed confirmed the search results were correct for the feed content that existed at query time in every case checked. Treat this column as reflecting feed volatility at measurement time, not fetcher or search correctness — Recall (100.00%, stable across every re-run) is the more meaningful number for this vendor.
