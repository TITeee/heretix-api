...
async function main() {
  console.log('Fetching Zabbix security advisories...');

  const job = await prisma.collectionJob.create({
    data: { source: 'advisory-zabbix', status: 'running', startedAt: new Date() },
  });

  try {
    const result = await runAdvisoryFetcher(new ZabbixFetcher());
