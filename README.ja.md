...
### RHEL/Oracle Linuxのモジュールストリームによる偽陽性（根本原因を修正済み。nodejs/postgresql以外は一部残存）

RHEL/Oracle Linuxは一部のソフトウェアをDNFモジュールストリーム——同一パッケージ名の下に複数のバージョン系統が並行共存する形式——で配布している（例: `postgresql:12`/`:13`/`:15`/`:16`/`:17`/`:18`、同様に`nodejs`, `mariadb`, `php`, `ruby`, `redis`, `podman`, `qemu-kvm`, `libvirt`等）。`RedHatFetcher`/`OracleLinuxFetcher`がパースしていた人間可読な`"<package> is earlier than <version>"`というOVAL criterionは上限のみを表現し下限を持たないため、新しい系統（例: postgresql:18が`18.4-2.module+el9.8.0...`で修正）向けの行が、無関係な古い系統のクエリ（例: postgresql 16.4）まで数値的に巻き込んでしまっていた——上記のソフトウェア群を含む155製品名で確認済み。

**根本原因を修正**: 実はOVALフィード自体には失われていた下限情報が存在していた——これまでfetcher側が読み捨てていた兄弟criterionの中に。RHEL/Oracle LinuxのOVALは、`"Module <名前>:N is enabled"`というcriterionと、それが管理下に置くパッケージ群のOR条件を、常に同じ`<criteria operator="AND">`の子要素として並べて配置する（両ベンダーの実フィードで直接確認済み）。[`redhat-fetcher.ts`](src/worker/redhat-fetcher.ts)と[`oracle-linux-fetcher.ts`](src/worker/oracle-linux-fetcher.ts)の`collectCriteria()`は、この木構造をたどってモジュールのメジャーバージョンを、それが適用範囲とする全criterionに伝播させ、インポート時点で直接`versionStart = "{major}.0"`をセットするようになった——製品名による許可リストは無く、自己限定的（OVALエントリにModule criterionが無い製品はこの変更の影響を受けない。両ベンダーの実データで`.module+`ビルドマーカーの有無と完全に1対1で一致することを確認済み）。`searchAdvisoryRpm()`（[`vulnerabilities.ts`](src/api/routes/vulnerabilities.ts) / [`search-helpers.ts`](src/utils/search-helpers.ts)の`matchesRpmVersionRange()`）は、他の`versionStart`と同様にこれを尊重する。修正前にインポート済みだった行は一度限りのバックフィル（`pnpm migrate:module-version-start`）で反映済みで、今後のインポートではnodejs/postgresqlに限らず全てのDNFモジュール製品に自動的に適用される。

**残存する問題**: 一部のアドバイザリは、パッケージがDNFモジュール化される以前のもの（あるいは`postgresql`のRHEL10のように、そもそもそのRHELバージョンではモジュール化されていない）で、抽出元となるModule criterionを持たない——例: RHEL9の`RHSA-2022:6595`、`nodejs 16.16.0-1.el9_0`（そのリリースでnodejsがモジュール化される前に発行されたもの）、`postgresql 9.2.24-...el7_9`（RHEL7自体がDNFモジュール化以前のOS）、`postgresql 13.23-...el9_8`（RHEL9のモジュール化前のベースライン）、`postgresql 16.14-...el10_2`（RHEL10ではモジュール化されずに配布）。こうした行には、ソースデータ上そもそも自然な下限が存在しない。`inferBareVersionStart()`（[`advisory-helpers.ts`](src/worker/advisory-helpers.ts)）が、`nodejs`と`postgresql`に限りその行自身の`versionEnd`の先頭メジャー桁にフォールバックする実装を追加し、それぞれ実際のクエリで前後の挙動を確認済み（`nodejs`は`pnpm validate:nodejs`でエンドツーエンド検証——[ACCURACY.md](ACCURACY.md#boundary-value-sweep-nodejs-module-streams--rhel--oracle-linux)参照。`postgresql`は`package=postgresql&version=9.2.10&ecosystem=oracle-linux`のヒット数が56件→29件に減り、残りは全て問い合わせた9.2系統内に収まることを確認）——`.module+`行全体に対する主要な修正のような一律のルールではなく、確認済みの2製品限定。なおpostgresqlでは単純な先頭1桁だけでは不十分だった: バージョン10より前は2桁がメジャーバージョン（`9.0`〜`9.6`、それぞれ互換性の無い別物）のため、`9.2.24-...`と`9.6.20-...`が最初は同じ`9.0`フロアに衝突していた——CVE-2019-10130（自身のRHSAタイトルが「postgresql:9.6 security update」）が`9.2.10`クエリに誤って一致することで発覚。`inferBareVersionStart()`は`9.x`系統だけ特別扱いし、`9.{minor}`まで含めたフロアにする。このフォールバックは、modular行とbare行が混在することが判明した残り約917組の(製品, ベンダー)ペア（`qemu-kvm`, `mysql`, `httpd`, `php`, `mariadb`, `golang`, `podman`, `libvirt`等）には**一般化していない**——それらについては、bare行が「同じメジャー系統に属するモジュール化以前の名残」（フロアを設けても安全）なのか、「メジャーバージョンをまたいで連続する正当なバージョン履歴」（フロアを設けると新たな偽陰性を生む）なのかが未検証のため。
