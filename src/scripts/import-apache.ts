...
async function main() {
  console.log('Fetching Apache httpd security advisories...');

  const job = await prisma.collectionJob.create({
    data: { source: 'advisory-apache', status: 'running', startedAt: new Date() },
  });

  try {
    const result = await runAdvisoryFetcher(new ApacheFetcher());
