...
| ecosystem の種類 | OSV 検索 | NVD 検索 | Advisory 検索 |
|---|---|---|---|
| `Red Hat:*` | `affectedVersions` exact match（結果なし: OSV に未収録） | スキップ | `rpmvercmp` でバージョン範囲比較（Red Hat OVAL データ） |
| `Ubuntu:*` / `Debian:*` / `Alpine:*` / `AlmaLinux:*` / `Rocky:*` / `CentOS:*` | `affectedVersions` exact match | スキップ | スキップ |
| `npm` / `PyPI` / `Go` / `Maven` 等 upstream | semver range 比較 | semver range 比較 | semver range 比較 |
| 未指定 | upstream のみ（ディストリビューション固有を除外） | semver range 比較 | semver range 比較 |
...
### ディストリビューション ecosystem 指定時はベンダーアドバイザリ検索がスキップされる（Red Hat を除く）
...
**例外**: `Red Hat:*` エコシステムが指定された場合は、Red Hat OVAL アドバイザリ（vendor: `red-hat`）のみが `rpmvercmp` バージョン比較付きで検索される。OSV に Red Hat エコシステムが存在しないため、これが RHEL 環境の主要な脆弱性検知経路となる。
