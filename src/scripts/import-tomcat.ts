...
async function main() {
  console.log('Fetching Apache Tomcat security advisories...');

  const job = await prisma.collectionJob.create({
    data: { source: 'advisory-tomcat', status: 'running', startedAt: new Date() },
  });

  try {
    const result = await runAdvisoryFetcher(new TomcatFetcher());
