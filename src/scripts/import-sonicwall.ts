...
async function main() {
  console.log('Fetching SonicWall PSIRT advisories...');

  const job = await prisma.collectionJob.create({
    data: { source: 'advisory-sonicwall', status: 'running', startedAt: new Date() },
  });

  try {
    const result = await runAdvisoryFetcher(new SonicWallFetcher());
