...
*PostgreSQL's numbers reflect a real production fix made during this investigation — see [RHEL/Oracle Linux module-stream false positives](README.md#rheloracle-linux-module-stream-false-positives-mitigated-for-the-default-search-fixed-for-postgresql-specifically) in README.md's Known Issues. Originally 5 FPs/0 FN (82.76%/100.00%/90.57%): 4 of the 5 FPs turned out to be a real bug (RHEL/Oracle Linux OVAL data for DNF module-stream software has no version lower bound, so a newer stream's fix version numerically swallowed this query for the older 16.x stream), not the "cross-source collision" originally assumed below. Fixed by excluding module-stream rows (`versionEnd` containing `.module+`) from the default (no-`ecosystem`) search — this removed the 4 bogus FPs but also cost 4 legitimate TPs that had no other searchable source (NVD has no version-range data at all for those specific CVEs), hence the new FNs. The remaining 1 FP (CVE-2017-8806) is a genuine, unrelated NVD product-identity mix-up (Debian's `postgresql-common` wrapper-script package mapped to product name "postgresql").*

*A follow-up fix (2026-07-30) properly resolves the root cause for `product=postgresql` instead of just excluding it: `importAdvisoryData()` now infers each module-stream row's own version floor (`versionStart = "{major}.0"`) when the vendor doesn't provide one, and `searchAdvisoryRpm()` — used for explicit `ecosystem=Red Hat:*`/`oracle-linux` queries, which the sweep above doesn't exercise — now actually checks `versionStart` (previously ignored even when populated). Verified directly: `ecosystem=oracle-linux&package=postgresql&version=16.4` no longer matches CVE-2026-6476 (PostgreSQL 17+ only) while still correctly matching CVE-2024-7348 (genuinely affects the 16.x stream too). This fix is not reflected in the table above since `validate:postgresql` only exercises the default (no-`ecosystem`) path; the table's FN count there is unchanged.*
