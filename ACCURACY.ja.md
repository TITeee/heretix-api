...
*2026-09-02に再度再現確認。Oracle Linux固有の2件の修正を追加で行った後の数値:*

3. *`OracleLinuxFetcher`が、OracleのFIPS検証済みビルドとkspliceライブパッチビルドという2つの並行トラックを、通常ビルドと同じ`(product, vendor)`バケツにマージしてしまっていた——このフェッチャーがこれまで無視していた`"<パッケージ> is fips patched"`/`"<パッケージ> is ksplice-based"`という兄弟criterionが目印だった。これらのトラックは独自の、無関係なepochを持つ（FIPSビルドは常にどの通常リリースよりも新しく見えるようepoch 10に固定されている）——マージされた結果、完全にパッチ済みの通常トラックのインストールが、一度も実行したことのないトラックの修正版と比較されて「脆弱」と誤判定されていた。`collectCriteria()`は、同じAND ブロック内の兄弟criterionにいずれかの目印があれば、そのcriterionに`isBuildVariant`という印を付けるようになり、該当行は取り込み自体からスキップされる。実データで確認: あるRHEL9相当イメージの`openssl`/`openssl-libs`/`gnutls`だけの誤検知数が、この修正だけで47件から0件になった。Red HatのOVALフィードにはどちらの目印も一件も存在しない（フィード全体で0件と確認済み）ため、これはOracle Linux固有の問題。*
4. *このスイープスクリプト自体（`validate-oracle-linux.ts`）が、静かに古い実装のままになっていた: 2026-09-01以前の形式である素の`ecosystem=oracle-linux`のままクエリしており、本番側はその後バージョン付きベンダー（`oracle-linux-<メジャー>`。項目2の経緯と[`rpmAdvisoryVendor()`](src/utils/search-helpers.ts)を参照）に移行済みだった——素の形式でも一致はするが、その変更以前の、ほぼ空になったレガシーバケツにしかヒットしない。このベンダーバージョニング変更以降初めてこのスイープを再実行したところRecallが0.02%まで崩壊し、一見上記③の破滅的な回帰に見えたが、実際には全てこのスクリプトが誤ったバケツに問い合わせていただけだった。各行自身のvendorから`ecosystem=Oracle Linux:<メジャー>`を導出し、`(product, OSメジャーバージョン)`単位でインデックス・クエリするよう修正した——`validate-redhat.ts`が単一の`Red Hat:9`にハードコードしたままスコープしている点も同様の見直しに値する（`RedHatFetcher`側にも同じ2026-09-01のベンダーバージョニング変更が入っているため、RHEL8専用のエントリがこのスイープから見えなくなっている可能性がある。未対応）。*

**Oracle LinuxのPrecision列についての注記**: 以前の測定値（86.88%〜87.01%、上記③修正前）は、Oracleの統合フィード`com.oracle.elsa-all.xml.bz2`が2回のフェッチ間でほぼリアルタイムに改訂される現象のせいだとされていた——これ自体は今も実在する効果だが（[既知の制限事項](#既知の制限事項)参照)、③の修正により、それが支配的な原因ではなかったことが分かった。FIPS/ksplice間の混線を取り除くだけで、1回の再実行でPrecisionが98.71%まで跳ね上がった——1つのバグで11ポイント以上の改善。100%までの残差は、スイープ自身が指摘しているフィード変動(ground truth取得とクエリの間でOracle側のフィードが変化する)で説明が付く規模であり、11ポイントもの差を覆っていた頃とは違い、この小さな残差であれば単独の説明として十分に妥当と言える。
