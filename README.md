...
- [x] OSV data collection
- [x] Fast version search via normalization
- [x] RESTful API
- [x] Prisma ERD generation
- [x] Ecosystem-wide bulk download
- [x] Batch search API
- [x] Approximate match fallback for non-semver versions
- [x] NIST NVD support (full mirror, incremental update, CPE mapping)
- [x] Multi-source DB schema (OSV/NVD separation)
- [x] CPE search API (`/search/cpe`)
- [x] Vulnerability master table (cross-source dedup, advisoryId support)
- [x] CISA KEV support
- [x] EPSS support
- [x] Accurate CPE point-version range conversion
- [x] Vendor advisory framework (`AdvisoryFetcher` interface)
- [x] Fortinet PSIRT (CSAF 2.0 / RSS, per-branch records)
- [x] Palo Alto Networks PSIRT (CSAF 2.0 / RSS, `vers:generic/` parsing)
- [x] Cisco PSIRT (openVuln API / OAuth 2.0, CSAF 2.0)
- [x] Automatic chunking for NVD 120-day date range limit
- [x] OSV upstream field CVE extraction & NVD master merge
- [x] OSV range event-pair fix (false positive bug fix)
- [x] Distro OSV `versions` exact match (root fix for `introduced: "0"` false positives)
- [x] Automatic data update scheduler (node-cron)
- [ ] Additional vendors (F5 BIG-IP, Microsoft MSRC, etc.)
- [ ] CVE.org integration
- [ ] GraphQL API
- [ ] Test suite
...
- [ ] CI/CD pipeline
- [ ] Prometheus metrics
- [ ] Package name normalization (e.g., NVD `xz` vs OSV `xz-utils`)
